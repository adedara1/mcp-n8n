...
export class N8NDocumentationMCPServer {
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async (request) => {
      ...
      let tools = this.isN8nCompatMode 
        ? [...n8nCompatTools] 
        : [...n8nDocumentationToolsFinal];
      const docTools = this.isN8nCompatMode ? n8nCompatTools : n8nDocumentationToolsFinal;
      const mgmtTools = this.isN8nCompatMode ? n8nCompatManagementTools : n8nManagementTools;
      
      // Determine which management tools to use based on compatibility mode
      const mgmtTools = this.isN8nCompatMode 
        ? n8nCompatManagementTools 
        : n8nManagementTools;
      ...
      if (isConfigured) {
        ...
        logger.debug(`Tool listing: ${tools.length} tools available (${tools.length - mgmtTools.length} documentation + ${mgmtTools.length} management)`);
      } else {
