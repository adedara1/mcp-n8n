...
## ✅ Latest Updates (v2.10.8 with SSE Integration)
...
1. **MCP Server** (`mcp/server.ts`)
   ...
   - Supports stdio (Claude Desktop), HTTP, and SSE modes
   - N8N compatibility mode for strict schema validation

2. **SSE Server** (`src/sse-server.ts`)
   - Server-Sent Events implementation for n8n MCP Server Trigger
   - Real-time streaming and async tool execution
...
The project implements MCP (Model Context Protocol) to expose n8n node documentation, source code, and examples to AI assistants with SSE support for real-time streaming. Key architectural components:
